...
for a = 0.01:0.05:0.51
    for b = 0.01:0.05:0.51
        for c = 0.01:0.05:0.51
            for d = 0.01:0.05:0.51
...
%            for d = 0.01:0.01
    ...
    b2a = find(var_mtx(:,4)==1&var_mtx(:,9)==1&sum(var_mtx(:,sel1),2)==0);
    b3 = find(sum(var_mtx,2)==0);
    
    % filename = sprintf('track_%.2f_%db.mat',a,j);
    filename = sprintf('track_%.2f_%.2f_%.2f_%.2f.mat',a,b,c,d);
